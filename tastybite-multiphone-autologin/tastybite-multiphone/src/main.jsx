...
const demoStaff = [
  { id: "s1", name: "Chef Arif", role: "Head Chef", login_role: "kitchen", email: "arif@tastybite.local", salary: 2400, hours: "9:00 AM - 6:00 PM", days: "Mon-Sat" },
  { id: "s2", name: "Riya", role: "Waiter", login_role: "waiter", email: "riya@tastybite.local", salary: 1400, hours: "10:00 AM - 7:00 PM", days: "Mon-Sat" },
  { id: "s3", name: "Sam", role: "Waiter", login_role: "waiter", email: "sam@tastybite.local", salary: 1400, hours: "1:00 PM - 10:00 PM", days: "Tue-Sun" },
  { id: "s4", name: "Priya", role: "Sous Chef", login_role: "kitchen", email: "priya@tastybite.local", salary: 1900, hours: "9:00 AM - 6:00 PM", days: "Mon-Sat" },
  { id: "s5", name: "Guest", role: "", login_role: "customer", email: "customer@tastybite.local", salary: 0, hours: "", days: "" },
  { id: "s6", name: "Admin", role: "Owner", login_role: "admin", email: "admin@tastybite.local", salary: 0, hours: "", days: "" },
];
...
function App() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      if (sess?.user?.email) {
        const found = await resolveLoginRole(sess.user.email);
        if (active && found) { setRole(found.role); localStorage.setItem(ROLE_STORAGE_KEY, found.role); }
      }
      if (!active) return;
      setSession(sess);
      setAuthReady(true);
  ...
  useEffect(() => {
    ...
    // IMPORTANT: refresh with the loader that matches the CURRENT device's role.
    // Previously this always called loadStaffData(), which loads every menu item
    // (including unavailable ones) and the full order list. That meant an action
    // on a staff device (e.g. admin/kitchen on a PC updating an order) pushed a
    // realtime event that made a customer's phone silently swap in the staff
    // view of the menu/orders instead of its own filtered "available items only"
    // view — the source of the phone vs. PC inconsistency.
    const refresh = () => (role === "customer" ? loadRestaurant() : loadStaffData());
    const channel = supabase.channel("tastybite-live")
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "staff", filter: `restaurant_id=eq.${restaurant.id}` }, () => loadStaffTable())
      .subscribe();
    return () => supabase.removeChannel(channel);
  }, [restaurant?.id, session?.user?.id, role]);
  ...
  async function bootstrap() {
    try {
      ...
      if (re) throw re;
      setRestaurant(r);
      await Promise.all([role === "customer" ? loadRestaurant(r) : loadStaffData(r), loadStaffTable(r)]);
    } catch (e) {
      setError(e.message);
      loadDemo();
      setDemo(true);
  ...
  // Staff & customer directory (Supabase "staff" table). Every row here is
  // someone who can log in — their login_role decides which workspace they
  // land on. This is what handleLogin/resolveLoginRole look up by email.
  async function loadStaffTable(r = restaurant) {
    if (!supabase || !r) return;
    const { data, error: se } = await supabase.from("staff").select("*").eq("restaurant_id", r.id).order("created_at");
    if (!se) setStaff(data || []);
  }
  async function addStaffToDb(item) {
    if (!supabase || !restaurant) { setStaff(prev => [...prev, { id: `s${Date.now()}`, ...item }]); return; }
    const { error: e } = await supabase.from("staff").insert({ restaurant_id: restaurant.id, ...item });
    if (e) setError(e.message); else loadStaffTable();
  }
  async function updateStaffInDb(id, patch) {
    if (!supabase || !restaurant) { setStaff(prev => prev.map(s => s.id === id ? { ...s, ...patch } : s)); return; }
    const { error: e } = await supabase.from("staff").update(patch).eq("id", id);
    if (e) setError(e.message); else loadStaffTable();
  }
  async function removeStaffFromDb(id) {
    if (!supabase || !restaurant) { setStaff(prev => prev.filter(s => s.id !== id)); return; }
    const { error: e } = await supabase.from("staff").delete().eq("id", id);
    if (e) setError(e.message); else loadStaffTable();
  }

  async function loadRestaurant(r = restaurant) {
    // Customers need their own orders too (for the "Your Orders" / "History" tabs),
    // not just tables and the available-only menu. This used to be missing, so a
    // customer's order list stayed empty even after successfully placing an order.
  ...
  // Looks up which workspace an email belongs to. In real (Supabase) mode this
  // reads the "staff" table, which holds every staff member AND every customer
  // the admin has added from Staff Management. In demo mode (no Supabase env
  // configured) it just matches against the built-in DEMO_USERS so the demo
  // still works without a backend.
  async function resolveLoginRole(email) {
    if (!supabase) {
      const match = Object.entries(DEMO_USERS).find(([, u]) => u.email.toLowerCase() === email.trim().toLowerCase());
      return match ? { role: match[0], name: match[1].name } : null;
    }
    const { data, error: se } = await supabase.from("staff").select("login_role,name").ilike("email", email.trim()).maybeSingle();
    if (se || !data) return null;
    return { role: data.login_role, name: data.name };
  }
  ...
  async function handleLogin({ email, password }) {
    ...
    try {
      if (supabase) {
        ...
        const found = await resolveLoginRole(data.user?.email || email);
        if (!found) {
          await supabase.auth.signOut();
          throw new Error("This email isn't set up yet. Ask your admin to add it in Staff Management.");
        }
        localStorage.setItem(ROLE_STORAGE_KEY, found.role);
        localStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify({ email: data.user?.email || email, role: found.role, name: data.user?.user_metadata?.full_name || found.name }));
        setRole(found.role);
        setSession(data.session);
      } else {
        const found = await resolveLoginRole(email);
        const demoUser = found ? DEMO_USERS[found.role] : null;
        if (!demoUser || password !== demoUser.password) {
          throw new Error("Email not recognised. Try one of the demo accounts shown below.");
        }
        const saved = { email: demoUser.email, role: found.role, name: demoUser.name };
  ...
  async function addMenuItem(item) {
    if (demo) { setMenu(ms => [...ms, { id: `m${Date.now()}`, ...item }]); return; }
    const { error: e } = await supabase.from("menu_items").insert({ restaurant_id: restaurant.id, ...item });
    if (e) setError(e.message); else loadStaffData();
  }
  async function updateMenuItem(id, patch) {
    if (demo) { setMenu(ms => ms.map(m => m.id === id ? { ...m, ...patch } : m)); return; }
    const { error: e } = await supabase.from("menu_items").update(patch).eq("id", id);
    if (e) setError(e.message); else loadStaffData();
  }
  async function deleteMenuItem(id) {
    if (demo) { setMenu(ms => ms.filter(m => m.id !== id)); return; }
  ...
  if (!session) return <Login onLogin={handleLogin} busy={authBusy} error={error} demo={!supabase} />;
  ...
  return <div className="app-shell">
    ...
    {role === "admin" && <Admin orders={orders} tables={tables} menu={menu} onLogout={logout} staff={staff} setStaff={setStaff} addStaffToDb={addStaffToDb} updateStaffInDb={updateStaffInDb} removeStaffFromDb={removeStaffFromDb} reviews={reviews} updateTable={updateTable} addMenuItem={addMenuItem} updateMenuItem={updateMenuItem} deleteMenuItem={deleteMenuItem} />}
  </div>;
}

function Login({ onLogin, busy, error, demo }) {
  ...
  return <div className="login-page">
    <div className="login-brand-panel">
      ...
      <div className="login-brand-copy">
        ...
        <p>Sign in with your email — we'll take you straight to your customer, waiter, kitchen or admin workspace automatically.</p>
      </div>
    ...
    <div className="login-card-wrap">
      <form className="login-card" onSubmit={submit}>
        <div className="login-card-head">
          <div><span className="login-eyebrow">TASTYBITE</span><h2>Welcome back</h2><p>Sign in — your workspace opens automatically.</p></div>
        ...
        {demo && <div className="demo-credentials"><strong>Demo accounts</strong>{Object.values(DEMO_USERS).map(u => <span key={u.email}>{u.email} / {u.password}</span>)}<small>Demo login is stored on this device until you log out.</small></div>}
        {error && <div className="login-error">{error}</div>}
        <button className="login-submit" disabled={busy}>{busy ? "Signing in…" : "Sign in"}<Icon name="arrow" size={18}/></button>
...
function Admin({orders,tables,menu,onLogout,staff=[],setStaff,addStaffToDb,updateStaffInDb,removeStaffFromDb,reviews=[],updateTable,addMenuItem,updateMenuItem,deleteMenuItem}) {
  const loginRoles = ["customer", "waiter", "kitchen", "admin"];
  const [newStaff, setNewStaff] = useState({ name: "", role: "", login_role: "customer", email: "", salary: "", hours: "", days: "" });
  const [editingStaffId, setEditingStaffId] = useState(null);
  const [editingMenuId, setEditingMenuId] = useState(null);
  const [newMenuItem, setNewMenuItem] = useState({ name: "", category: "", price: "", description: "" });
  const patchStaff = (id, field, value) => {
    const v = field === "salary" ? Number(value) || 0 : value;
    setStaff && setStaff(prev => prev.map(s => s.id === id ? { ...s, [field]: v } : s));
    updateStaffInDb && updateStaffInDb(id, { [field]: v });
  };
  const patchMenu = (id, field, value) => updateMenuItem && updateMenuItem(id, { [field]: field === "price" ? Number(value) || 0 : value });
  const addMenuFromForm = () => {
    if (!newMenuItem.name.trim() || !addMenuItem) return;
    addMenuItem({ name: newMenuItem.name, category: newMenuItem.category || "Mains", price: Number(newMenuItem.price) || 0, description: newMenuItem.description });
    setNewMenuItem({ name: "", category: "", price: "", description: "" });
  };
  // Adds either a staff member or a customer — same directory, same form.
  // Whatever login_role is picked decides which workspace this email opens
  // into next time they sign in.
  const addStaffMember = () => {
    if (!newStaff.name.trim() || !newStaff.email.trim()) return;
    const entry = { ...newStaff, salary: Number(newStaff.salary) || 0 };
    setStaff && setStaff(prev => [...prev, { id: `s${Date.now()}`, ...entry }]);
    addStaffToDb && addStaffToDb(entry);
    setNewStaff({ name: "", role: "", login_role: "customer", email: "", salary: "", hours: "", days: "" });
  };
  const removeStaffMember = (id) => {
    setStaff && setStaff(prev => prev.filter(s => s.id !== id));
    removeStaffFromDb && removeStaffFromDb(id);
  };
  ...
  return <div className="staff-page admin-page"><SideNav onLogout={onLogout} active={section} onNavigate={setSection}/><div className="staff-main"><TopBar onLogout={onLogout}/>{section === 1 ? <div className="section-panel"><div className="staff-heading"><div><h1>Orders</h1><p>All orders across the restaurant.</p></div></div><section className="panel recent-panel full-panel"><table><thead><tr><th>#</th><th>Table</th><th>Items</th><th>Total</th><th>Status</th><th>Time</th></tr></thead><tbody>{orders.map(o=><tr key={o.id}><td>#{o.id}</td><td>{o.table_id==="takeaway"?"Takeaway":`Table ${tables.find(t=>t.id===o.table_id)?.table_number||o.table_id}`}</td><td>{o.order_items?.length||2} items</td><td>{money(o.total)}</td><td><span className={`pill ${o.status}`}>{o.status}</span></td><td>{new Date(o.created_at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</td></tr>)}</tbody></table></section></div> : section === 2 ? <div className="section-panel"><div className="staff-heading"><div><h1>Menu Management</h1><p>Every dish currently on the menu. Click a row to edit it.</p></div></div><section className="panel recent-panel full-panel"><table><thead><tr><th>Item</th><th>Category</th><th>Price</th><th>Description</th><th></th></tr></thead><tbody>{menu.map(m=>editingMenuId===m.id?<tr key={m.id} className="editing-row"><td><input value={m.name} onChange={e=>patchMenu(m.id,"name",e.target.value)}/></td><td><input value={m.category} onChange={e=>patchMenu(m.id,"category",e.target.value)}/></td><td><input type="number" value={m.price} onChange={e=>patchMenu(m.id,"price",e.target.value)}/></td><td><input value={m.description||""} onChange={e=>patchMenu(m.id,"description",e.target.value)}/></td><td><button className="row-remove" onClick={()=>setEditingMenuId(null)}>Done</button></td></tr>:<tr key={m.id}><td>{m.name}</td><td>{m.category}</td><td>{money(m.price)}</td><td>{m.description}</td><td><button className="row-edit" onClick={()=>setEditingMenuId(m.id)}>Edit</button><button className="row-remove" onClick={()=>deleteMenuItem&&deleteMenuItem(m.id)}>Remove</button></td></tr>)}</tbody></table></section><section className="panel add-staff-panel"><PanelHead title="Add Menu Item"/><div className="add-staff-form"><input placeholder="Name" value={newMenuItem.name} onChange={e=>setNewMenuItem({...newMenuItem,name:e.target.value})}/><input placeholder="Category" value={newMenuItem.category} onChange={e=>setNewMenuItem({...newMenuItem,category:e.target.value})}/><input placeholder="Price" type="number" value={newMenuItem.price} onChange={e=>setNewMenuItem({...newMenuItem,price:e.target.value})}/><input placeholder="Description" value={newMenuItem.description} onChange={e=>setNewMenuItem({...newMenuItem,description:e.target.value})}/><button onClick={addMenuFromForm}>Add Item</button></div></section></div> : section === 7 ? <div className="section-panel"><div className="staff-heading"><div><h1>Staff &amp; Customer Directory</h1><p>Everyone who can log in — staff and customers. Login Access controls which workspace their email opens. Click a row to edit it.</p></div></div><section className="panel recent-panel full-panel"><table><thead><tr><th>Name</th><th>Email</th><th>Job Title</th><th>Login Access</th><th>Salary/mo</th><th>Working Hours</th><th>Days</th><th>Rating</th><th></th></tr></thead><tbody>{staff.map(s=>editingStaffId===s.id?<tr key={s.id} className="editing-row"><td><input value={s.name} onChange={e=>patchStaff(s.id,"name",e.target.value)}/></td><td><input type="email" value={s.email||""} onChange={e=>patchStaff(s.id,"email",e.target.value)}/></td><td><input value={s.role} onChange={e=>patchStaff(s.id,"role",e.target.value)}/></td><td><select value={s.login_role||"customer"} onChange={e=>patchStaff(s.id,"login_role",e.target.value)}>{loginRoles.map(r=><option key={r} value={r}>{roleLabels[r]}</option>)}</select></td><td><input type="number" value={s.salary} onChange={e=>patchStaff(s.id,"salary",e.target.value)}/></td><td><input value={s.hours} onChange={e=>patchStaff(s.id,"hours",e.target.value)}/></td><td><input value={s.days} onChange={e=>patchStaff(s.id,"days",e.target.value)}/></td><td>{avgRating(s.id)} ★</td><td><button className="row-remove" onClick={()=>setEditingStaffId(null)}>Done</button></td></tr>:<tr key={s.id}><td>{s.name}</td><td>{s.email||"—"}</td><td>{s.role}</td><td><span className={`pill role-${s.login_role||"customer"}`}>{roleLabels[s.login_role]||"Customer"}</span></td><td>{money(s.salary)}</td><td>{s.hours}</td><td>{s.days}</td><td>{avgRating(s.id)} ★</td><td><button className="row-edit" onClick={()=>setEditingStaffId(s.id)}>Edit</button><button onClick={()=>removeStaffMember(s.id)} className="row-remove">Remove</button></td></tr>)}</tbody></table></section><section className="panel add-staff-panel"><PanelHead title="Add Staff or Customer"/><p className="panel-sub">Add anyone who needs to log in — staff or a customer. Create their login in Supabase Authentication with this same email first, then add them here so the app knows which workspace to open for them.</p><div className="add-staff-form"><input placeholder="Name" value={newStaff.name} onChange={e=>setNewStaff({...newStaff,name:e.target.value})}/><input placeholder="Email (must match their Supabase login)" type="email" value={newStaff.email} onChange={e=>setNewStaff({...newStaff,email:e.target.value})}/><label className="field-label">Login Access<select value={newStaff.login_role} onChange={e=>setNewStaff({...newStaff,login_role:e.target.value})}>{loginRoles.map(r=><option key={r} value={r}>{roleLabels[r]}</option>)}</select></label><input placeholder="Job Title (e.g. Waiter, Head Chef — optional for customers)" value={newStaff.role} onChange={e=>setNewStaff({...newStaff,role:e.target.value})}/><input placeholder="Monthly Salary" type="number" value={newStaff.salary} onChange={e=>setNewStaff({...newStaff,salary:e.target.value})}/><input placeholder="Working Hours (e.g. 9:00 AM - 6:00 PM)" value={newStaff.hours} onChange={e=>setNewStaff({...newStaff,hours:e.target.value})}/><input placeholder="Days (e.g. Mon-Sat)" value={newStaff.days} onChange={e=>setNewStaff({...newStaff,days:e.target.value})}/><button onClick={addStaffMember}>Add to Directory</button></div></section></div> : section === 9 ? <div className="section-panel"><div className="staff-heading"><div><h1>Customer Reviews</h1><p>Ratings and feedback from customers, for the restaurant and for staff.</p></div></div><div className="kpi-row admin-kpis" style={{gridTemplateColumns:"repeat(3,1fr)"}}><div className="admin-kpi green"><div className="kpi-icon"><Icon name="star"/></div><span>Restaurant Rating</span><strong>{avgRating("restaurant")} ★</strong></div><div className="admin-kpi blue"><div className="kpi-icon"><Icon name="users"/></div><span>Total Reviews</span><strong>{reviews.length}</strong></div><div className="admin-kpi purple"><div className="kpi-icon"><Icon name="chart"/></div><span>Staff Reviewed</span><strong>{new Set(reviews.filter(r=>r.target!=="restaurant").map(r=>r.target)).size}</strong></div></div><section className="panel recent-panel full-panel"><table><thead><tr><th>Customer</th><th>Rated</th><th>Rating</th><th>Comment</th><th>Date</th></tr></thead><tbody>{reviews.length===0?<tr><td colSpan="5">No reviews yet.</td></tr>:reviews.map(r=><tr key={r.id}><td>{r.name}</td><td>{r.target==="restaurant"?"Restaurant":staff.find(s=>s.id===r.target)?.name||"Staff"}</td><td>{"★".repeat(r.rating)}</td><td>{r.text}</td><td>{new Date(r.created_at).toLocaleDateString()}</td></tr>)}</tbody></table></section></div> : section === 8 ? <div className="section-panel"><div className="staff-heading"><div><h1>Tables</h1><p>Change a table's status directly.</p></div></div><div className="table-grid">{tables.map(t=><div className="table-card" key={t.id}><div className="table-card-head"><span>Table {t.table_number}</span><em className={t.status}>{t.status}</em></div><select value={t.status} onChange={e=>updateTable&&updateTable(t.id,e.target.value)}><option value="available">available</option><option value="occupied">occupied</option><option value="ordering">ordering</option><option value="cleaning">cleaning</option></select></div>)}</div></div> : section !== 0 ? <Placeholder title={adminSections[section] || "Coming soon"}/> : <><div className="admin-heading"><div><h1>Good Morning, Admin!</h1><p>Here's what's happening at your restaurant today.</p></div><div className="date-actions"><button><Icon name="calendar"/><span>Apr 26, 2025</span><small>Saturday</small></button><button>Today⌄</button><button>Main Branch⌄</button><button type="button" className="admin-logout-btn" onClick={onLogout}><Icon name="logout" size={16}/><span>Log out</span></button></div></div><div className="kpi-row admin-kpis">{[["$","Total Sales",money(sales),"12%","green"],["orders","Total Orders",orders.length||42,"8%","red"],["users","Total Customers",38,"10%","purple"],["chart","Average Order Value",money(59.20),"6%","orange"],["chart","Net Profit",money(1248.50),"14%","blue"]].map(([i,l,v,p,c])=><div className={`admin-kpi ${c}`} key={l}><div className="kpi-icon"><Icon name={i}/></div><span>{l}</span><strong>{v}</strong><small>↑ {p} <i>vs yesterday</i></small></div>)}</div><div className="dashboard-grid"><section className="panel sales-panel"><PanelHead title="Sales Overview" right="Last 7 Days⌄"/><div className="chart"><div className="y-labels"><span>$4,000</span><span>$3,000</span><span>$2,000</span><span>$1,000</span><span>$0</span></div><svg viewBox="0 0 700 220" preserveAspectRatio="none"><defs><linearGradient id="fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopOpacity=".22"/><stop offset="1" stopOpacity="0"/></linearGradient></defs><path d="M25 170 L125 145 L225 160 L325 135 L425 150 L525 115 L625 130 L680 80 L680 210 L25 210 Z" fill="url(#fill)"/><polyline points="25,170 125,145 225,160 325,135 425,150 525,115 625,130 680,80" fill="none" stroke="currentColor" strokeWidth="3"/><polyline points="25,195 125,175 225,190 325,175 425,185 525,165 625,170 680,145" fill="none" stroke="currentColor" strokeOpacity=".35" strokeWidth="2"/>{[25,125,225,325,425,525,625,680].map((x,i)=><circle key={x} cx={x} cy={i===7?80:[170,145,160,135,150,115,130][i]} r="4" fill="currentColor"/>)}</svg><div className="x-labels"><span>Apr 20</span><span>Apr 21</span><span>Apr 22</span><span>Apr 23</span><span>Apr 24</span><span>Apr 25</span><span>Apr 26</span></div></div></section><section className="panel category-panel"><PanelHead title="Sales by Category"/><div className="donut"><div><strong>{money(sales)}</strong><span>Total Sales</span></div></div><div className="legend">{[["Pizza","28%"],["Burgers","22%"],["Pasta","18%"],["Salads","12%"],["Drinks","8%"],["Desserts","7%"],["Others","5%"]].map((x,i)=><span key={x[0]}><i className={`dot d${i}`}/>{x[0]}<b>{x[1]}</b></span>)}</div></section><section className="panel recent-panel"><PanelHead title="Recent Orders" right="View All →"/><table><thead><tr><th>#</th><th>Table</th><th>Items</th><th>Total</th><th>Status</th><th>Time</th></tr></thead><tbody>{orders.slice(0,5).map(o=><tr key={o.id}><td>#{o.id}</td><td>{o.table_id==="takeaway"?"Takeaway":`Table ${tables.find(t=>t.id===o.table_id)?.table_number||o.table_id}`}</td><td>{o.order_items?.length||2} items</td><td>{money(o.total)}</td><td><span className={`pill ${o.status}`}>{o.status}</span></td><td>{new Date(o.created_at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</td></tr>)}</tbody></table></section><section className="panel inventory-panel"><PanelHead title="Inventory Alerts" right="View All →"/><table><thead><tr><th>Item</th><th>Current Stock</th><th>Status</th></tr></thead><tbody>{[["🍅","Tomatoes","2 kg","Low"],["🧀","Cheese","1 kg","Low"],["🍗","Chicken Breast","1.5 kg","Low"],["🥬","Lettuce","3 kg","OK"],["🫒","Olive Oil","1 bottle","Low"]].map(x=><tr key={x[1]}><td>{x[0]} &nbsp;{x[1]}</td><td>{x[2]}</td><td><span className={`pill ${x[3]==="OK"?"ok":"low"}`}>{x[3]}</span></td></tr>)}</tbody></table></section><section className="panel top-items"><PanelHead title="Top Selling Items" right="View All →"/><table><thead><tr><th>#</th><th>Item</th><th>Category</th><th>Quantity</th><th>Revenue</th></tr></thead><tbody>{top.map((x,i)=><tr key={x.name}><td>{i+1}</td><td><span className="food-thumb" style={{backgroundImage:`url(${foodImages[x.name]||foodImages["Margherita Pizza"]})`}}/>{x.name}</td><td>{x.cat}</td><td>{x.qty}</td><td>{money(x.rev)}</td></tr>)}</tbody></table></section><section className="panel payment-panel"><PanelHead title="Sales by Payment Method"/><div className="payment-wrap"><div className="payment-donut"><strong>{money(sales)}</strong><span>Total Sales</span></div><div className="legend">{[["Cash","32%"],["Card","48%"],["UPI","15%"],["Online","5%"]].map((x,i)=><span key={x[0]}><i className={`dot p${i}`}/>{x[0]}<b>{x[1]}</b></span>)}</div></div></section><section className="panel quick-panel"><PanelHead title="Quick Actions"/><div className="quick-actions"><button onClick={()=>setSection(2)}><Icon name="plus"/><span>Add Menu Item</span></button><button onClick={()=>setSection(8)}><Icon name="box"/><span>Manage Tables</span></button><button onClick={()=>setSection(10)}><Icon name="chart"/><span>View Reports</span></button><button onClick={()=>setSection(7)}><Icon name="users"/><span>Add Staff</span></button></div></section></div><div className="quick-stats"><div><span>Today's Revenue</span><strong>{money(sales)}</strong><small>↑ 12%</small></div><div><span>Total Orders</span><strong>42</strong><small>↑ 8%</small></div><div><span>Avg. Order Value</span><strong>{money(59.2)}</strong><small>↑ 6%</small></div><div><span>Active Tables</span><strong>6 / 8</strong><small>75% Occupied</small></div></div></>}</div></div>;
}
