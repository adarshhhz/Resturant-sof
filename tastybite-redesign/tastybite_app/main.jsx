...
function App() {
  ...
  useEffect(() => { if (supabase) bootstrap(); else loadDemo(); }, []);
  useEffect(() => { if (!supabase || !restaurant) return; const channel = supabase.channel("tastybite-live").on("postgres_changes", { event: "*", schema: "public", table: "orders", filter: `restaurant_id=eq.${restaurant.id}` }, () => loadStaffData()).on("postgres_changes", { event: "*", schema: "public", table: "order_items" }, () => loadStaffData()).subscribe(); return () => supabase.removeChannel(channel); }, [restaurant?.id]);

  function loadDemo() { setRestaurant({ id: "demo", name: "TastyBite" }); setTables(demoTables); setMenu(demoMenu); setOrders(demoOrders); setLoading(false); }
  async function bootstrap() {
    try { const { data: r, error: re } = await supabase.from("restaurants").select("*").limit(1).single(); if (re) throw re; setRestaurant(r); await (role === "customer" ? loadRestaurant(r) : loadStaffData(r)); }
    catch (e) { setError(e.message); loadDemo(); setDemo(true); }
    finally { setLoading(false); }
  }
  async function loadRestaurant(r = restaurant) {
    const [{ data: ts, error: te }, { data: ms, error: me }] = await Promise.all([supabase.from("restaurant_tables").select("*").eq("restaurant_id", r.id).order("table_number"), supabase.from("menu_items").select("*").eq("restaurant_id", r.id).eq("is_available", true).order("category").order("name")]);
    if (te) throw te; if (me) throw me; setTables(ts || []); setMenu(ms || []);
  }
  async function loadStaffData(r = restaurant) {
    const [{ data: ts, error: te }, { data: ms, error: me }, { data: os, error: oe }] = await Promise.all([supabase.from("restaurant_tables").select("*").eq("restaurant_id", r.id).order("table_number"), supabase.from("menu_items").select("*").eq("restaurant_id", r.id).order("category").order("name"), supabase.from("orders").select("*, order_items(*)").eq("restaurant_id", r.id).order("created_at", { ascending: false }).limit(100)]);
    if (te) throw te; if (me) throw me; if (oe) throw oe; setTables(ts || []); setMenu(ms || []); setOrders(os || []);
  }
  const switchRole = r => { setRole(r); history.replaceState({}, "", `?role=${r}${r === "customer" && tableId ? `&table=${tableId}` : ""}`); if (!demo && supabase) { r === "customer" ? loadRestaurant() : loadStaffData(); } };
  async function submitOrder() {
    if (!cart.length || !tableId) return;
    if (demo) { const id = String(1030 + orders.length); setOrders(o => [{ id, table_id: tableId, status: "new", total: cart.reduce((s, x) => s + Number(x.price) * x.qty, 0), created_at: new Date().toISOString(), order_items: cart.map((x, i) => ({ id: `${id}-${i}`, item_name: x.name, quantity: x.qty })) }, ...o]); setCart([]); return; }
    const total = cart.reduce((s, x) => s + Number(x.price) * x.qty, 0);
    const { data: order, error: oe } = await supabase.from("orders").insert({ restaurant_id: restaurant.id, table_id: tableId, status: "new", subtotal: total, tax: 0, total }).select().single(); if (oe) return setError(oe.message);
    const { error: ie } = await supabase.from("order_items").insert(cart.map(x => ({ order_id: order.id, menu_item_id: x.id, item_name: x.name, quantity: x.qty, unit_price: x.price, notes: "" }))); if (ie) return setError(ie.message);
    await supabase.from("restaurant_tables").update({ status: "ordering" }).eq("id", tableId); setCart([]);
  }
  async function updateOrder(id, status) { if (demo) { setOrders(os => os.map(o => o.id === id ? { ...o, status } : o)); return; } const { error: e } = await supabase.from("orders").update({ status }).eq("id", id); if (e) setError(e.message); else loadStaffData(); }
  async function updateTable(id, status) { if (demo) { setTables(ts => ts.map(t => t.id === id ? { ...t, status } : t)); return; } const { error: e } = await supabase.from("restaurant_tables").update({ status }).eq("id", id); if (e) setError(e.message); else loadStaffData(); }

  if (loading) return <div className="loading-screen"><Logo/><span>Connecting to TastyBite…</span></div>;
  return <div className="app-shell">
    {error && <div className="error-toast">{error}</div>}
    <RoleSwitcher role={role} onChange={switchRole}/>
    {role === "customer" && <Customer menu={menu} tables={tables} tableId={tableId} setTableId={setTableId} cart={cart} setCart={setCart} submitOrder={submitOrder}/>} 
    {role === "waiter" && <Waiter tables={tables} orders={orders} updateOrder={updateOrder} updateTable={updateTable}/>} 
    {role === "kitchen" && <Kitchen orders={orders} tables={tables} updateOrder={updateOrder}/>} 
    {role === "admin" && <Admin orders={orders} tables={tables} menu={menu}/>} 
  </div>;
}

function RoleSwitcher({ role, onChange }) {
  return <div className="role-switcher"><Logo light={role === "kitchen"}/><div className="role-links">{["customer","waiter","kitchen","admin"].map(r => <button className={role === r ? "selected" : ""} key={r} onClick={() => onChange(r)}>{r}</button>)}</div></div>;
}

function SideNav({ role = "admin" }) {
  const items = role === "kitchen" ? [["home","Kitchen Dashboard"],["orders","New Orders"],["clock","In Progress"],["check","Ready"],["orders","Completed"],["history","Orders History"],["box","Kitchen Stations"],["settings","Settings"]] : [["home","Dashboard"],["orders","Orders"],["menu","Menu Management"],["box","Inventory"],["box","Purchases"],["box","Suppliers"],["box","Expenses"],["users","Staff Management"],["box","Tables"],["users","Customers"],["chart","Reports"],["settings","Settings"]];
  return <aside className="side-nav"><Logo light={role === "kitchen"}/><div className="side-items">{items.map(([icon,label], i) => <button key={label+i} className={i===0 ? "active" : ""}><Icon name={icon === "history" ? "clock" : icon}/><span>{label}</span>{label === "Orders" && <b className="badge">5</b>}</button>)}</div><div className="side-bottom"><div className="profile-mini"><div className="avatar">A</div><div><b>{role === "kitchen" ? "Chef Arif" : "Admin"}</b><span>{role === "kitchen" ? "Head Chef" : "System Administrator"}</span></div><Icon name="arrow" size={16}/></div><button><Icon name="logout"/> <span>Log out</span></button></div></aside>;
}
