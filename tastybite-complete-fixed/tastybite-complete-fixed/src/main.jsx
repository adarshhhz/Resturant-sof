...
function App() {
  ...
  return <div className="app-shell">
    ...
    {role === "customer" && <Customer menu={menu} tables={tables} tableId={initialTable} setTableId={id => { window.__tastybite_table = id; history.replaceState({}, "", `?role=customer&table=${id}`); }} cart={cart} setCart={setCart} submitOrder={submitOrder} onLogout={logout} user={session.user} />}
    ...
    {role === "admin" && <Admin orders={orders} tables={tables} menu={menu} onLogout={logout} />}
  </div>;
}
...
function SideNav({ role = "admin", onLogout }) {
...
function Customer({ menu, tables, tableId, setTableId, cart, setCart, submitOrder, onLogout, user }) {
...
function Admin({orders,tables,menu,onLogout}) {
